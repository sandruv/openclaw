...
import { BasicInformation } from "../../new/form/components/BasicInformation"
import { AdvancedInformation } from "../../new/form/components/AdvancedInformation"
import { useToast } from "@/hooks/useToast"
import { User } from "@/types/clients"
import { EditUserFormSkeleton } from "./loaders/EditUserFormSkeleton"

// User form data type (matches structure expected by form components)
...
export function EditUserForm({ userId }: EditUserFormProps) {
  ...
  const { updateUser, getUser, isFetchingUser } = useUserStore()
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (userData) {
          ...
          setFormData({
            ...
            sophistication_id: userData.sophistication_id || 0,
            is_user_vip: userData.is_user_vip || false,
    ...
    loadUser()
    fetchClients()
  }, [userId, getUser, fetchClients, toast])
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await updateUser(userId, {
        ...
        is_user_vip: formData.is_user_vip,
        sophistication_id: formData.sophistication_id,
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-170px)] gap-4">
