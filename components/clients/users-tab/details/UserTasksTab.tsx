...
export function UserTasksTab({ userId }: UserTasksTabProps) {
  ...
  const fetchUserTasks = useCallback(async (page = 1, limit = 15, search = '') => {
    try {
      setIsLoading(true)
      const response = await getTasks({
        ...
        filters: {
          ...
          // Include all statuses (1-7) to show New, Open, Pending, In Progress, Resolved, Closed, and Archived
          status: [
            TaskStatusType.New.toString(), 
            TaskStatusType.InProgress.toString(), 
            TaskStatusType.OnHold.toString(), 
            TaskStatusType.PendingEndUser.toString(), 
            TaskStatusType.UserResponded.toString(), 
            TaskStatusType.Closed.toString(), 
            TaskStatusType.Archived.toString()
          ]
        }
      })
  ...
  useEffect(() => {
    fetchUserTasks(1, pageSize, '').finally(() => {
      setInitialLoad(false)
    })
  }, [userId, pageSize, fetchUserTasks])

  useEffect(() => {
    const unsubscribeUpdate = pusherService.subscribeToTaskUpdates(() => {
      fetchUserTasks(pagination.page, pageSize, searchTerm)
    })
    
    const unsubscribeCreate = pusherService.subscribeToTaskCreation(() => {
      fetchUserTasks(pagination.page, pageSize, searchTerm)
    })
    
    const unsubscribeDelete = pusherService.subscribeToTaskDeletion(() => {
      fetchUserTasks(pagination.page, pageSize, searchTerm)
    })
  ...
  }, [pagination.page, pageSize, searchTerm, fetchUserTasks])

  const handlePageChange = async (page: number) => {
    try {
      if (page !== pagination.page) {
        setIsSearching(true)
        await fetchUserTasks(page, pageSize, searchTerm)
  ...
  const handleSearchChange = (value: string) => {
    ...
    searchTimeout.current = setTimeout(async () => {
      try {
        await fetchUserTasks(1, pageSize, value)
      } catch (error) {
  ...
  const handleSearchSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsSearching(true)
      await fetchUserTasks(1, pageSize, searchTerm)
      setIsSearching(false)
    } catch (error) {
      console.error('Failed to search tasks', error)
      setIsSearching(false)
  ...
  const handleClearSearch = async () => {
    ...
    try {
      setIsSearching(true)
      await fetchUserTasks(1, pageSize, '')
      setIsSearching(false)
    } catch (error) {
  ...
  return (
    <div className="container max-w-full mt-2">
      ...
      <Table>
        ...
        <TableBody>
          {tasks.length === 0 && !isSearching && !initialLoad ? (
            <TableRow>
              <TableCell colSpan={6} className="h-24 text-center">
                <div className="flex items-center justify-center gap-2 text-muted-foreground">
                  <span>No tasks found{searchTerm ? ' matching your search' : ' for this user'}.</span>
                </div>
              </TableCell>
            </TableRow>
          ) : (
            tasks.map((task: Task) => {
              ...
              return (
                <TableRow
                  ...
                  onClick={() => { setRouteLoading(true); router.push(`/tasks/${task.id}`) }}
                >
